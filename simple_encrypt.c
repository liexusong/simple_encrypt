...
PHP_FUNCTION(se_encrypt)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        DES_encipher(inbuf, binstr + start, key);
    }
...
/* {{{ */
static char *se_hexstr_decrypt(char *str, int len, int *newlen, char *key)
{
    int i, count, start;
    char *binstr, *retstr;
    int binlen;
    ...
    for (i = 0; i < count; i++) {
        start = i * 8;
        DES_decipher((const char *)(binstr+start), retstr+start, key);
    }
...
PHP_FUNCTION(se_decrypt)
{
    ...
    retstr = se_hexstr_decrypt(str, len, &retlen, key);
    if (retstr == NULL) {
        RETURN_FALSE;
    }
